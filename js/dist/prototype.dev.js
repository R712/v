...
//
//  고정 레이아웃 요소
var headerGNB = $('#header');
var appContainer = $('.home');
var vibePlayer = $('#player'); //
//  스크롤된 상황인지 체크하여 UI 통 제어
//

appContainer.on('scroll', function () {
  var scrollY = appContainer.scrollTop();
  console.log(scrollY);
...
$('.btn_option_now, .btn_option, .btn_drop_menu').click(function (e) {
  e.preventDefault();
  var elm = $(this);
  var elmNav = elm.next('.ly_option, .layer_drop_menu');
...
headerGNB.on('click', '.profile_area .profile', function (e) {
  e.preventDefault();
  var elm = $(this);
  var elmNav = elm.next('.ly_option');
  $('.ly_option').not(elmNav).hide();
  elmNav.toggle();
}); // 서브 탭 메뉴

$('.tab_menu').on('click', '.tab_item', function () {
  var motherTab = $(this).closest('.tab_menu');
...
var searchHtml = headerGNB.find('.search_area').clone();
headerGNB.find('.btn_search').after(searchHtml); //  여기까지
...
$('.VueCarousel-navigation-next').click(function () {
  var motherSection = $(this).parent('div').find('.scroll_list');
  var sumWidth = 0;
  motherSection.find('.list_item').each(function () {
    sumWidth += parseInt($(this).width());
  });
  motherSection.animate({
    left: '-=' + parseInt(motherSection.closest('div').width() + 20, 10) + 'px'
  }, 150);
  var motherPosition = motherSection.position();
  var motherX = parseInt(motherPosition.left) * -1 + motherSection.closest('div').width();
...
$('#chk_all, .label_check').click(function () {
  if ($('#chk_all').is(':checked')) {
    ...
    var selectedTrack = $('.tracklist').find(':checked').length;
...
if ($('.today_headline')) {
  var copyThumb = $('.today_headline').find('img').attr('src');
...
if ($('.menu_list.owl-carousel').length) {
  var onPosition = $('.menu_item.on').parent().index();
